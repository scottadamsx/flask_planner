...
export function calculatePayPeriod(txDateStr, paySchedule) {
  const txDate = parseDate(txDateStr);
  const scheduleType = paySchedule?.type || "biweekly";
  const anchor = parseDate(paySchedule?.anchorDate || toDateStr(new Date()));
  let start;
  let end;

  if (scheduleType === "semimonthly") {
    if (txDate.getDate() < 15) {
      start = new Date(txDate.getFullYear(), txDate.getMonth(), 1);
      end = new Date(txDate.getFullYear(), txDate.getMonth(), 14);
    } else {
      start = new Date(txDate.getFullYear(), txDate.getMonth(), 15);
      end = new Date(txDate.getFullYear(), txDate.getMonth() + 1, 0);
    }
    ...
    const interval =
      scheduleType === "weekly" ? 7 : scheduleType === "custom" ? paySchedule?.customDays || 14 : 14;
    const daysSince = Math.floor((txDate - anchor) / 86400000);
    const periodNum = Math.floor(daysSince / interval);
