...
import { expandReminders, formatDisplayDate, toDateStr } from "../utils";
...
export default function DashboardPage() {
  ...
  const [data, setData] = useState({
    reminders: [],
    journal: [],
    config: { categories: [], recurringBills: [] }
  });

  useEffect(() => {
    const load = async () => {
      const [reminders, journal, config] = await Promise.all([
        getJson("/loadData"),
        getJson("/loadJournal"),
        getJson("/loadBudgetConfig")
      ]);
  ...
  return (
    <div className="db-grid">
      <div className="db-card col-8">
        ...
        <div className="todo-sections">
          <div className="todo-section">
            <h4 className="section-label">Due Today</h4>
              ...
              {todayReminders.map((r) => (
                <div className="db-list-item" key={`${r.id}-${r.date}`}>
                  <div className="db-list-item-content">
                    <div className="db-list-item-title">{r.name || r.text}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="todo-section todo-section-upcoming">
            <h4 className="section-label">Upcoming by Week</h4>
            <div className="weekly-scroll-container">
              {weeks.map((w) => (
                <div className="weekly-card" key={w.label}>
                  <div className="db-list-item-title weekly-card-title">{w.label}</div>
                  {w.reminders.length === 0 && <p className="no-entries-compact">No tasks this week.</p>}
                  {w.reminders.map((r) => (
                    <div className="db-list-item weekly-reminder-item" key={`${r.id}-${r.date}`}>
                      <div className="db-list-item-content">
                        <div className="db-list-item-title weekly-reminder-name">{r.name || r.text}</div>
      ...
      <div className="db-card col-4">
        ...
        <div className="stat-grid">
          ...
          <div className="stat-item">
            ...
            <span className="stat-value">{data.config?.categories?.length || 0}</span>
          </div>
        </div>
      </div>
      ...
      <div className="db-card col-6">
        ...
        {lastEntry ? (
          <div className="db-list-item journal-snippet">
            <div className="db-list-item-title">{lastEntry.title}</div>
            <p className="journal-snippet-body">{lastEntry.entry?.slice(0, 150)}</p>
